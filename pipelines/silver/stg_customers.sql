...
SELECT DISTINCT customer_email AS customer_key, 'online' AS customer_type FROM bronze.shopify_orders
),
square_loyalty AS (
SELECT DISTINCT customer_phone AS customer_key, 'loyalty' AS customer_type FROM bronze.square_transactions
...
FROM bronze.square_transactions
WHERE customer_phone IS NULL
),
combined AS (
SELECT * FROM shopify_email
UNION ALL
SELECT * FROM square_loyalty
UNION ALL
